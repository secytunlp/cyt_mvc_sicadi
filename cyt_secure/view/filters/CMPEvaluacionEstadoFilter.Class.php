...
/**
 * componente filter para evaluaciones estados.
 *
 * @author Marcos
 * @since 03-12-2013
 *
 */
class CMPEvaluacionEstadoFilter extends CMPFilter{
	...
	public function __construct( $id="filter_evaluacionesEstado"){

		parent::__construct($id);


		$this->setComponent("CMPEvaluacionEstadoGrid");
		...
		$findEvaluacion = CYTSecureComponentsFactory::getFindEvaluacion(new Evaluacion(), CYT_LBL_EVALUACION, "", "evaluacionEstado_filter_evaluacion_oid", "evaluacion.oid", "");
		$findEvaluacion->setMinWidth("372px;");
		$this->addField( $findEvaluacion );
		
		$fieldEstado = FieldBuilder::buildFieldSelect (CYT_LBL_SOLICITUD_ESTADO, "estado.oid", CYTSecureUtils::getEvaluacionEstadosItems(), null, null, null, "--Seleccionar--" );
	...
	protected function fillCriteria( $criteria ){
		...
		//filtramos por evaluacion.
		$evaluacion = $this->getEvaluacion();
		if($evaluacion!=null && $evaluacion->getOid()!=null){
			$criteria->addFilter("evaluacion_oid", $evaluacion->getOid(), "=" );
		}
		
		//filtramos por estado de evaluacion.
