...
class CYTSecureUtils {
	...
	public static function getFacultadesItems($mostradas="") {
		if ($mostradas) {
			$oCriteria = new CdtSearchCriteria();
			$filter = new CdtSimpleExpression("cd_facultad in (".$mostradas.")");
			$oCriteria->setExpresion($filter);
		}
		return self::getFilterOptionItems( CYTSecureManagerFactory::getFacultadManager(), "oid", "ds_facultad","","","","cd_facultad",$oCriteria);
